...
_AGENT_RULES = """\
## Rules
- THINK BEFORE YOU ACT. Before calling ANY tool, you MUST write a `<thought>...</thought>` block to reason about what you are trying to do, what tools you need, and the step-by-step plan.
...
- AFTER A TOOL FAILS (timeout, error, "Unknown action", "not found"), DO NOT GO SILENT. Reflect on the failure in your `<thought>` block, debug the issue, and either retry with a fix (e.g. correct args, longer-running form) or explicitly tell them "this didn't work". A failed tool is not a stopping condition — only a successful one is.
...
_AGENT_RULES = """\
## Base rules
- THINK BEFORE YOU ACT. Before calling ANY tool, you MUST write a `<thought>...</thought>` block to reason about what you are trying to do.
...
- After a tool fails, reflect on the failure in your `<thought>` block, debug the issue, and retry with a concrete fix. Do not give up immediately.
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_tasks": "- ```manage_tasks``` — Create and manage scheduled background tasks (recurring AI jobs). Args (JSON): {\"action\": \"list|create|edit|delete|pause|resume|run|sleep_and_resume\", ...}. Use `sleep_and_resume` to pause execution and wake up later.",
